...
//        return false;
//    }
    if (HAL_I2C_Master_Receive(hi2c, REG_IDENTIFICATION_MODEL_ID, *device_id, 1, 100) != HAL_OK)
    {
    	return false;
    }
...
//        return false;
//    }
    if (HAL_I2C_Master_Receive(hi2c, REG_VHV_CONFIG_PAD_SCL_SDA_EXTSUP_HV, *vhv_config_scl_sda, 1, 100) != HAL_OK)
	{
		return false;
	}
...
//        return false;
//    }
    if (HAL_I2C_Master_Transmit(hi2c, REG_VHV_CONFIG_PAD_SCL_SDA_EXTSUP_HV, *vhv_config_scl_sda, 1, 100) != HAL_OK)
	{
		return false;
	}
...
//    success &= i2c_write_addr8_data8(0x80, 0x00);
    ...
    if (HAL_I2C_Master_Transmit(hi2c, 0x88, (uint8_t*)0x00, 1, 100) == HAL_OK) success = true;

    if (HAL_I2C_Master_Transmit(hi2c, 0x80, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
    if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
    if (HAL_I2C_Master_Transmit(hi2c, 0x00, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;

    if (HAL_I2C_Master_Receive(hi2c, 0x91, &stop_variable, 1, 100) != HAL_OK) success = false;
    if (HAL_I2C_Master_Transmit(hi2c, 0x00, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
    if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
    if (HAL_I2C_Master_Transmit(hi2c, 0x80, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
...
//        return false;
//    }
    if (HAL_I2C_Master_Transmit(hi2c, 0x83, (uint8_t*)0x00, 1, 100) != HAL_OK)
    {
    	return false;
    }
//    do {
...
//    } while (success && (strobe == 0));
    ...
    while (1)
    {
    	success = HAL_I2C_Master_Receive(hi2c, 0x83, &strobe, 1, 100);
    	if ((success != HAL_OK) || (strobe != 0)) break;
    }
...
//    }
    ...
    if (HAL_I2C_Master_Transmit(hi2c, 0x83, (uint8_t*)0x01, 1, 100) != HAL_OK)
    {
    	return false;
    }
...
//    success &= i2c_write_addr8_data8(0x80, 0x01);
    ...
    if (HAL_I2C_Master_Transmit(hi2c, 0x80, (uint8_t*)0x01, 1, 100) == HAL_OK) 					success = true;
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x01, 1, 100) != HAL_OK) 					success = false;
	if (HAL_I2C_Master_Transmit(hi2c, 0x00, (uint8_t*)0x00, 1, 100) != HAL_OK) 					success = false;
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x06, 1, 100) != HAL_OK) 					success = false;
	if (HAL_I2C_Master_Receive(hi2c, 0x83, &tmp_data8, 1, 100) != HAL_OK) 						success = false;
	if (HAL_I2C_Master_Transmit(hi2c, 0x83, (uint8_t*)(tmp_data8 | 0x04), 1, 100) != HAL_OK) 	success = false;
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x07, 1, 100) != HAL_OK) 					success = false;
	if (HAL_I2C_Master_Transmit(hi2c, 0x81, (uint8_t*)0x01, 1, 100) != HAL_OK) 					success = false;
	if (HAL_I2C_Master_Transmit(hi2c, 0x80, (uint8_t*)0x01, 1, 100) != HAL_OK) 					success = false;
...
//    success &= i2c_write_addr8_data8(0x94, 0x6b);
	if (HAL_I2C_Master_Transmit(hi2c, 0x94, (uint8_t*)0x6b, 1, 100) != HAL_OK) 					success = false;
    if (!success) {
        return false;
    }
...
//    success &= i2c_read_addr8_data32(0x90, &tmp_data32);
	if (HAL_I2C_Master_Receive(hi2c, 0x90, &tmp_data32, 1, 100) != HAL_OK) 						success = false;
...
//    success &=i2c_write_addr8_data8(0x80, 0x00);
	...
	if (HAL_I2C_Master_Transmit(hi2c, 0x81, (uint8_t*)0x00, 1, 100) != HAL_OK) 					success = false;
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x06, 1, 100) != HAL_OK) 					success = false;
	if (HAL_I2C_Master_Receive(hi2c, 0x83, &tmp_data8, 1, 100) != HAL_OK) 						success = false;
	if (HAL_I2C_Master_Transmit(hi2c, 0x83, (uint8_t*)(tmp_data8 | 0xfb), 1, 100) != HAL_OK) 	success = false;
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x01, 1, 100) != HAL_OK) 					success = false;
	if (HAL_I2C_Master_Transmit(hi2c, 0x00, (uint8_t*)0x01, 1, 100) != HAL_OK) 					success = false;
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x00, 1, 100) != HAL_OK) 					success = false;
	if (HAL_I2C_Master_Transmit(hi2c, 0x80, (uint8_t*)0x00, 1, 100) != HAL_OK) 					success = false;
...
//    }
	...
	if (HAL_I2C_Master_Receive(hi2c, REG_GLOBAL_CONFIG_SPAD_ENABLES_REF_0, good_spad_map, 6, 100) != HAL_OK)
	{
		return false;
	}
...
//    success &= i2c_write_addr8_data8(REG_GLOBAL_CONFIG_REF_EN_START_SELECT, SPAD_START_SELECT);
    bool success = false;
    if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x01, 1, 100) == HAL_OK) success = true;
    if (HAL_I2C_Master_Transmit(hi2c, REG_DYNAMIC_SPAD_REF_EN_START_OFFSET, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
    if (HAL_I2C_Master_Transmit(hi2c, REG_DYNAMIC_SPAD_NUM_REQUESTED_REF_SPAD, (uint8_t*)0x2C, 1, 100) != HAL_OK) success = false;
    if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
    if (HAL_I2C_Master_Transmit(hi2c, REG_GLOBAL_CONFIG_REF_EN_START_SELECT, (uint8_t*)SPAD_START_SELECT, 1, 100) != HAL_OK) success = false;
...
//        return false;
//    }
    if (HAL_I2C_Master_Transmit(hi2c, REG_GLOBAL_CONFIG_SPAD_ENABLES_REF_0, spad_map, SPAD_MAP_ROW_COUNT, 100) != HAL_OK)
    {
    	return false;
    }
...
//    success &= i2c_write_addr8_data8(0x09, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x09, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x10, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x10, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x11, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x11, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x24, 0x01);
	if (HAL_I2C_Master_Transmit(hi2c, 0x24, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x25, 0xFF);
	if (HAL_I2C_Master_Transmit(hi2c, 0x25, (uint8_t*)0xFF, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x75, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x75, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0xFF, 0x01);
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x4E, 0x2C);
	if (HAL_I2C_Master_Transmit(hi2c, 0x4E, (uint8_t*)0x2C, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x48, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x48, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x30, 0x20);
	if (HAL_I2C_Master_Transmit(hi2c, 0x30, (uint8_t*)0x20, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0xFF, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x30, 0x09);
	if (HAL_I2C_Master_Transmit(hi2c, 0x30, (uint8_t*)0x09, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x54, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x54, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x31, 0x04);
	if (HAL_I2C_Master_Transmit(hi2c, 0x31, (uint8_t*)0x04, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x32, 0x03);
	if (HAL_I2C_Master_Transmit(hi2c, 0x32, (uint8_t*)0x03, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x40, 0x83);
	if (HAL_I2C_Master_Transmit(hi2c, 0x40, (uint8_t*)0x83, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x46, 0x25);
	if (HAL_I2C_Master_Transmit(hi2c, 0x46, (uint8_t*)0x25, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x60, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x60, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x27, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x27, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x50, 0x06);
	if (HAL_I2C_Master_Transmit(hi2c, 0x50, (uint8_t*)0x06, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x51, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x51, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x52, 0x96);
	if (HAL_I2C_Master_Transmit(hi2c, 0x52, (uint8_t*)0x96, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x56, 0x08);
	if (HAL_I2C_Master_Transmit(hi2c, 0x56, (uint8_t*)0x08, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x57, 0x30);
	if (HAL_I2C_Master_Transmit(hi2c, 0x57, (uint8_t*)0x30, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x61, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x61, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x62, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x62, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x64, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x64, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x65, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x65, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x66, 0xA0);
	if (HAL_I2C_Master_Transmit(hi2c, 0x66, (uint8_t*)0xA0, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0xFF, 0x01);
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x22, 0x32);
	if (HAL_I2C_Master_Transmit(hi2c, 0x22, (uint8_t*)0x32, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x47, 0x14);
	if (HAL_I2C_Master_Transmit(hi2c, 0x47, (uint8_t*)0x14, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x49, 0xFF);
	if (HAL_I2C_Master_Transmit(hi2c, 0x49, (uint8_t*)0xFF, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x4A, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x4A, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0xFF, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x7A, 0x0A);
	if (HAL_I2C_Master_Transmit(hi2c, 0x7A, (uint8_t*)0x0A, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x7B, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x7B, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x78, 0x21);
	if (HAL_I2C_Master_Transmit(hi2c, 0x78, (uint8_t*)0x21, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0xFF, 0x01);
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x23, 0x34);
	if (HAL_I2C_Master_Transmit(hi2c, 0x23, (uint8_t*)0x34, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x42, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x42, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x44, 0xFF);
	if (HAL_I2C_Master_Transmit(hi2c, 0x44, (uint8_t*)0xFF, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x45, 0x26);
	if (HAL_I2C_Master_Transmit(hi2c, 0x45, (uint8_t*)0x26, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x46, 0x05);
	if (HAL_I2C_Master_Transmit(hi2c, 0x46, (uint8_t*)0x05, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x40, 0x40);
	if (HAL_I2C_Master_Transmit(hi2c, 0x40, (uint8_t*)0x40, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x0E, 0x06);
	if (HAL_I2C_Master_Transmit(hi2c, 0x0E, (uint8_t*)0x06, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x20, 0x1A);
	if (HAL_I2C_Master_Transmit(hi2c, 0x20, (uint8_t*)0x1A, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x43, 0x40);
	if (HAL_I2C_Master_Transmit(hi2c, 0x43, (uint8_t*)0x40, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0xFF, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x34, 0x03);
	if (HAL_I2C_Master_Transmit(hi2c, 0x34, (uint8_t*)0x03, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x35, 0x44);
	if (HAL_I2C_Master_Transmit(hi2c, 0x35, (uint8_t*)0x44, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0xFF, 0x01);
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x31, 0x04);
	if (HAL_I2C_Master_Transmit(hi2c, 0x31, (uint8_t*)0x04, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x4B, 0x09);
	if (HAL_I2C_Master_Transmit(hi2c, 0x4B, (uint8_t*)0x09, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x4C, 0x05);
	if (HAL_I2C_Master_Transmit(hi2c, 0x4C, (uint8_t*)0x05, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x4D, 0x04);
	if (HAL_I2C_Master_Transmit(hi2c, 0x4D, (uint8_t*)0x04, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0xFF, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x44, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x44, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x45, 0x20);
	if (HAL_I2C_Master_Transmit(hi2c, 0x45, (uint8_t*)0x20, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x47, 0x08);
	if (HAL_I2C_Master_Transmit(hi2c, 0x47, (uint8_t*)0x08, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x48, 0x28);
	if (HAL_I2C_Master_Transmit(hi2c, 0x48, (uint8_t*)0x28, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x67, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x67, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x70, 0x04);
	if (HAL_I2C_Master_Transmit(hi2c, 0x70, (uint8_t*)0x04, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x71, 0x01);
	if (HAL_I2C_Master_Transmit(hi2c, 0x71, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x72, 0xFE);
	if (HAL_I2C_Master_Transmit(hi2c, 0x72, (uint8_t*)0xFE, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x76, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x76, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x77, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0x77, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0xFF, 0x01);
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x0D, 0x01);
	if (HAL_I2C_Master_Transmit(hi2c, 0x0D, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0xFF, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x80, 0x01);
	if (HAL_I2C_Master_Transmit(hi2c, 0x80, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x01, 0xF8);
	if (HAL_I2C_Master_Transmit(hi2c, 0x01, (uint8_t*)0xF8, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0xFF, 0x01);
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x8E, 0x01);
	if (HAL_I2C_Master_Transmit(hi2c, 0x8E, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0x00, 0x01);
	if (HAL_I2C_Master_Transmit(hi2c, 0x00, (uint8_t*)0x01, 1, 100) != HAL_OK) success = false;
//    success &= i2c_write_addr8_data8(0xFF, 0x00);
	if (HAL_I2C_Master_Transmit(hi2c, 0xFF, (uint8_t*)0x00, 1, 100) != HAL_OK) success = false;
...
//    }
	...
	if (HAL_I2C_Master_Transmit(hi2c, REG_SYSTEM_INTERRUPT_CONFIG_GPIO, (uint8_t*)0x04, 1, 100) != HAL_OK)
	{
		return false;
	}
...
//    }
    ...
    if (HAL_I2C_Master_Receive(hi2c, REG_GPIO_HV_MUX_ACTIVE_HIGH, &gpio_hv_mux_active_high, 1, 100) != HAL_OK)
    {
    	return false;
    }
...
//        return false;
//    }
    if (HAL_I2C_Master_Transmit(hi2c, REG_GPIO_HV_MUX_ACTIVE_HIGH, &gpio_hv_mux_active_high, 1, 100) != HAL_OK)
	{
		return false;
	}
...
//        return false;
//    }
    if (HAL_I2C_Master_Transmit(hi2c, REG_SYSTEM_INTERRUPT_CLEAR, (uint8_t*)0x01, 1, 100) != HAL_OK)
	{
		return false;
	}
...
//    return i2c_write_addr8_data8(REG_SYSTEM_SEQUENCE_CONFIG, sequence_step);
	if (HAL_I2C_Master_Transmit(hi2c, REG_SYSTEM_SEQUENCE_CONFIG, &sequence_step, 1, 100) != HAL_OK)
	{
		return false;
	}
	return true;
...
//    }
    ...
    if (HAL_I2C_Master_Transmit(hi2c, REG_SYSTEM_SEQUENCE_CONFIG, &sequence_config, 1, 100) != HAL_OK)
	{
		return false;
	}
...
//    }
    ...
    if (HAL_I2C_Master_Transmit(hi2c, REG_SYSRANGE_START, &sysrange_start, 1, 100) != HAL_OK)
	{
		return false;
	}
...
//    } while (success && ((interrupt_status & 0x07) == 0));
    ...
    while (1)
    {
    	success = HAL_I2C_Master_Receive(hi2c, REG_RESULT_INTERRUPT_STATUS, &interrupt_status, 1, 100);
    	if ((success != HAL_OK) || ((interrupt_status & 0x07)) != 0) break;
    }
...
//    }
    ...
    if (HAL_I2C_Master_Transmit(hi2c, REG_SYSRANGE_START, (uint8_t*)0x00, 1, 100) != HAL_OK)
	{
		return false;
	}
...
//    return i2c_write_addr8_data8(REG_SLAVE_DEVICE_ADDRESS, addr & 0x7F);
    if (HAL_I2C_Master_Transmit(hi2c, REG_SLAVE_DEVICE_ADDRESS, &addr, 1, 100) != HAL_OK)
    {
    	return false;
    }
    return true;
