...
int main(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, GPIO_PIN_15, 1);
  HAL_Delay(20);
  while (!initVL53L0X(1, &lox1, &hi2c1))
  {
    HAL_GPIO_WritePin(LEDR_GPIO_Port, LEDR_Pin, 0); // on led
    printf("Failed 1!\n");
  }
  setAddress(0x30, &lox1);
...
//  HAL_GPIO_WritePin(GPIOC, GPIO_PIN_15, 0);
  ...
  while (!initVL53L0X(1, &lox2, &hi2c1))
  {
    HAL_GPIO_WritePin(LEDR_GPIO_Port, LEDR_Pin, 0); // on led
    printf("Failed 3!\n");
  }
//  setAddress(0x40, &lox2);
//
//  while (!initVL53L0X(1, &lox2, &hi2c1))
//  {
//    HAL_GPIO_WritePin(LEDR_GPIO_Port, LEDR_Pin, 0); // on led
...
//  HAL_Delay(20);
  ...
  //  HAL_Delay(20);
  //
  //  if (setAddress(0x40, &lox2)) {
  //	  flag = true;
  //	  HAL_GPIO_WritePin(LedOnBoard_GPIO_Port, LedOnBoard_Pin, 0);
  //  }
  //  while (!initVL53L0X(1, &lox2, &hi2c1))
  //  {
  //    HAL_GPIO_WritePin(LEDB_GPIO_Port, LEDB_Pin, GPIO_PIN_RESET); //on led
  //    printf("Failed 3!\n");
  //    HAL_Delay(1000);
  //  }
  //  uint8_t curAddr = getAddress(&lox1);
  ...
  while (1)
  {
    ...
    //
    //	}
    uint16_t distance1 = readRangeSingleMillimeters(&lox1);
    uint16_t distance2 = readRangeSingleMillimeters(&lox2);
    if (flag)
    {
      if (distance1 > distance2 && distance1 < 8000 && distance2 < 8000)
      {
        HAL_GPIO_TogglePin(GPIOB, LEDR_Pin);
      }
      else if (distance1 < distance2 && distance1 < 8000 && distance2 < 8000)
      {
        HAL_GPIO_TogglePin(GPIOB, LEDB_Pin);
      } else {
...
static void MX_GPIO_Init(void)
{
  ...
  GPIO_InitStruct.Pin = LEDB_Pin | LEDG_Pin | LEDR_Pin | MPU_BOOT_Pin | MPU_RST_Pin | XSHUT_MCU1_1_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
